...
class AIAssetStatementParser:
    ...
    def parse_file_with_ai(self, file_content, file_type='pdf', filename=''):
        ...
        try:
            ...
            prompt = """
            你是一个专业的财务审计机器人。请分析上传的资产文件，提取所有资产信息。
            要求：
            1. 识别并归类：
               - liquid_assets (可动资产): 如 TNG, ASNB, 银行存款, RKT现金, 股票账户余额。
               - illiquid_assets (不可动资产): 专门提取 KWSP (EPF)。
               - stocks_my: 马股 (4位代码)。
               - stocks_us: 美股 (大写字母代码)。
               - gold: 识别实物黄金价值。
            2. 请确保提取 shares (股数) 和 avgPrice (成本价)。
            3. 直接返回纯 JSON，格式如下：
            {
    ...
    def _process_and_calculate(self, ai_text):
        ...
        try:
            ...
            return {'success': False, 'error': f'数据统计异常: {str(e)}'}

    def get_financial_advice(self, db_data):
        if not self.google_api_key: return "未配置AI密钥"
        try:
            model = genai.GenerativeModel('gemini-1.5-flash')
            prompt = f"我的资产数据: {json.dumps(db_data)}。请用中文简短分析我的财务状况并给一个建议。"
